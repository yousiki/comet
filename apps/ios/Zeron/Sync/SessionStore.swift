...
final class SessionStore {
    ...
    /// Chunked upload of one staged image to the host device, returning the
    /// durable absolute path on that device (what the refs trailer carries).
    /// Real sends go through sendWithTransfers; this upload-first flow
    /// remains for demo mode, where no host resolves pending:// refs.
    ...
    /// The queued-attachment send (PR #168): the command
