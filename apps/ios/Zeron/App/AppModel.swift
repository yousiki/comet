...
final class AppModel {
    ...
    /// Queued attachments are the only real path (no-backward-compat fork:
    /// every host defers commands with pending:// refs). Demo mode has no
    /// host to resolve them, so it keeps the upload-first flow.
