...
export const FRAME = {
  ...
  hello: 0x01,
  /** `{after}` — request backfill rows with `seq > after`. A historical
   * `excludeOwn` header key is ignored: device ids are not bound to user
   * identity in Organization-shared rooms, so rows are never filtered. */
  rowsReq: 0x03,
