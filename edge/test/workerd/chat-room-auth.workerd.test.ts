...
describe("ChatRoom HTTP rows authorization", () => {
  it("keeps legacy chat2 rows owner-only", async () => {
    const stub = env.CHAT_ROOM.get(env.CHAT_ROOM.idFromName("legacy-rows-owner"));

    expect((await stub.fetch(request("/rows", "alice", "GET"))).status).toBe(404);
    expect(
      (
        await stub.fetch(
          request("/rows?device=alice-dev&batchId=before-claim", "alice", "POST", {
    ...
    expect(
      (
        await stub.fetch(
          request("/rows?device=alice-dev&batchId=alice-1", "alice", "POST", {
