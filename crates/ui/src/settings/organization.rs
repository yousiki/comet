//! Settings → Organization: the shared-organization management surface.
//!
//! - Organizations: every membership listed; switching (or creating/deleting)
...
//! - Members: the organization roster with invite-by-email, role changes, and
...
const ORGANIZATION_RPC_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(20);
...
pub enum OrganizationMutationKind {
    Select,
    Create,
    Delete,
}
...
impl OrganizationMutation {
    pub fn kind(&self) -> OrganizationMutationKind {
        match self {
            Self::Select { .. } => OrganizationMutationKind::Select,
            Self::Create { .. } => OrganizationMutationKind::Create,
            Self::Delete { .. } => OrganizationMutationKind::Delete,
        }
    }
...
#[derive(Debug, Clone)]
pub enum OrganizationPageEvent {
    /// Shell must run the shared destructive preflight before issuing this RPC.
    RequestRuntimeChange(OrganizationMutation),
}

impl EventEmitter<OrganizationPageEvent> for OrganizationPage {}
...
pub struct OrganizationPage {
    ...
    invite_admin: bool,
    new_organization: Option<Entity<ComposerInput>>,
    confirm_delete: bool,
...
impl OrganizationPage {
    pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
        let invite = cx.new(|cx| ComposerInput::new("member@example.com", cx));
    ...
    fn refresh(&mut self, cx: &mut Context<Self>) {
        ...
        let Some(engine) = self.state.read(cx).engine().cloned() else {
            self.organizations = Loadable::Error("Engine not connected".into());
            self.members = Loadable::Error("Engine not connected".into());
            cx.notify();
            return;
        };
        let organization_id = self.current_organization_id(cx);
        self.error = None;
        self.organizations = Loadable::Loading;
        self.members = if organization_id.is_some() {
            Loadable::Loading
        } else {
        ...
        self.load_task = Some(cx.spawn(async move |this, cx| {
            ...
            this.update(cx, |page, cx| {
                page.load_task = None;
                page.organizations = match organizations {
                    Ok(value) => Loadable::Ready(sort_memberships(parse_organizations(&value))),
                    Err(err) => Loadable::Error(format!("Loading organizations failed: {err}")),
                };
                match members {
                    Some(Ok(value)) => {
    ...
    /// Fire one non-profile-changing auth RPC. Organization profile mutations are
    ...
    fn run(
        ...
        let rpc = Tokio::spawn(cx, async move {
            match tokio::time::timeout(
                ORGANIZATION_RPC_TIMEOUT,
                engine.client().call(method, params),
            )
            .await
            {
                Ok(result) => result,
                Err(_) => Err(zeron_rpc::RpcError::Transport(format!(
                    ...
                    ORGANIZATION_RPC_TIMEOUT.as_secs()
                ))),
            }
        });
        self.action_task = Some(cx.spawn(async move |this, cx| {
            let result = match rpc.await {
                ...
                Err(error) => Err(zeron_rpc::RpcError::Transport(format!(
                    "Organization request task failed: {error}"
                ))),
            };
            this.update(cx, |page, cx| {
    ...
    fn submit_invite(&mut self, cx: &mut Context<Self>) {
        ...
        let Some(organization_id) = self.current_organization_id(cx) else {
            return;
        };
        ...
        self.run(
            ...
            serde_json::json!({ "organizationId": organization_id, "email": email, "role": role }),
            SuccessAction::Invite,
            cx,
        );
    }

    fn submit_new_organization(&mut self, cx: &mut Context<Self>) {
        if self.busy || self.load_task.is_some() {
            return;
        }
        let Some(input) = self.new_organization.as_ref() else {
            return;
        };
        let name = input.read(cx).text().trim().to_string();
        if !organization_name_valid(&name) {
            self.error = Some("Organization names must be 1-64 characters.".into());
            cx.notify();
            return;
        }
        // CREATE_ORGANIZATION also selects the new organization. The shell must approve the
        // destructive handoff before this intent can become an RPC.
        self.error = None;
        self.info = None;
        cx.emit(OrganizationPageEvent::RequestRuntimeChange(
            OrganizationMutation::Create { name },
        ));
    }

    fn render_organizations(&mut self, theme: &Theme, cx: &mut Context<Self>) -> AnyElement {
        let current = self.current_organization_id(cx);
        ...
        match organizations {
            Loadable::Idle | Loadable::Loading => {
                card = card.child(widgets::card_row(theme, true).child(div().flex_1().child(
                    popover::skeleton_rows("organizations-skeleton", theme, 3, cx.entity_id(), cx),
                )));
            }
            ...
            Loadable::Ready(organizations) if organizations.is_empty() => {
                card = card.child(
                    widgets::card_row(theme, true).child(
                        div()
                            .text_size(px(12.5))
                            .text_color(theme.text_muted)
                            .child("You don't belong to any organizations yet."),
                    ),
                );
            }
            Loadable::Ready(organizations) => {
                for (ix, organization) in organizations.into_iter().enumerate() {
                    let is_current = Some(&organization.organization_id) == current.as_ref();
                    let mut row = widgets::card_row(theme, ix == 0)
                        ...
                        .child(crate::avatar::organization_avatar(
                        ...
                        .child(
                            div()
                                .flex_1()
                                .min_w_0()
                                .child(widgets::row_title(theme, organization.name.clone()))
                    ...
                    if is_current {
                        ...
                        let organization_name: SharedString = organization.name.into();
                        row = row.child(
                            widgets::ghost_action(theme)
                                .id(SharedString::from(format!("switch-organization-{ix}")))
                                .hover(|s| widgets::ghost_hover(theme, s))
        ...
        // New-organization affordance: inline input when armed, button otherwise.
        let first = self
            .organizations
            .ready()
            .is_none_or(|organizations| organizations.is_empty());
        card = card.child(match &self.new_organization {
            Some(input) => widgets::card_row(theme, first)
                .child(div().flex_1().child(input.clone()))
                .child(
                    widgets::ghost_action(theme)
                        .id("new-organization-create")
                        .hover(|s| widgets::ghost_hover(theme, s))
                        .when(busy, |button| button.opacity(0.45))
                        .when(!busy, |button| {
                            button.on_click(
                                cx.listener(|this, _, _, cx| this.submit_new_organization(cx)),
                            )
                        })
                        .child("Create"),
                ),
                ...
                widgets::ghost_action(theme)
                    ...
                    .when(!busy, |button| {
                        button.on_click(cx.listener(|this, _, _, cx| {
                            let input = cx.new(|cx| ComposerInput::new("Organization name", cx));
                            let sub = cx.subscribe(&input, |this: &mut Self, _, event, cx| {
                                if matches!(event, ComposerInputEvent::Submitted) {
                                    this.submit_new_organization(cx);
                                }
                            });
                            this._subs.push(sub);
                            this.new_organization = Some(input);
                            this.error = None;
                            cx.notify();
                        }))
                    })
    ...
    fn render_members(&mut self, theme: &Theme, cx: &mut Context<Self>) -> AnyElement {
        ...
        let me = self
            ...
            .map(|u| u.id.clone())
            .unwrap_or_default();
        let organization_id = self.current_organization_id(cx).unwrap_or_default();
                ...
                card = card.child(
                    widgets::card_row(theme, true).child(
                        div()
                            .text_size(px(12.5))
                            .text_color(theme.text_muted)
                            .child("No members found in this organization."),
                    ),
                );
            }
            Loadable::Ready(members) => {
                for (ix, member) in members.into_iter().enumerate() {
                    ...
                    if member.user_id == me {
                        ...
                        let organization_for_role = organization_id.clone();
                        row = row.child(
                            widgets::ghost_action(theme)
                                ...
                                .when(!busy, |button| {
                                    button.on_click(cx.listener(move |this, _, _, cx| {
                                        this.run(
                                            ...
                                            serde_json::json!({
                                                "organizationId": organization_for_role.clone(),
    ...
    fn render_danger(&mut self, theme: &Theme, cx: &mut Context<Self>) -> Option<AnyElement> {
        ...
        self.current_organization_id(cx)?;
        let busy = self.busy || self.load_task.is_some();
        Some(
                        ...
                        .child(
                            div()
                                ...
                                .child(widgets::meta_line(
                                    ...
                                    vec![div()
                                        .child(SharedString::from(
                                            "Deleting an organization removes it for every member. Sessions stay in each device's local data.",
                                        ))
                                        .into_any_element()],
                                )),
                        )
    ...
    fn render_confirmation(
        ...
        if let Some(member) = self.confirm_remove.clone() {
            ...
            let body = format!(
                "Remove {display_name} ({}) from this organization? They won't be able to renew access. An already-open connection may remain active until it reconnects or its session expires.",
                member.email
            );
            let card = popover::dialog_card(theme)
                ...
                .child(
                    div()
                        ...
                        .gap(px(8.0))
                        .child(
                            popover::btn_ghost(theme, "Cancel", "organization-remove-cancel")
                                .id("organization-remove-cancel")
                        ...
                        .child(
                            popover::btn_danger(theme, "Remove member")
                                .id("organization-remove-confirm")
            ...
            return Some(popover::modal(
                "organization-remove-member-dialog",
                viewport,
                card,
            ));
        }
        ...
        if self.confirm_delete {
            ...
            let body = format!(
                "Delete {organization_name} for every member? Memberships cannot renew access, though already-open connections may remain active until they reconnect or expire. This cannot be undone."
            );
            let card = popover::dialog_card(theme)
                .child(popover::dialog_title(theme, "Delete organization?"))
                .child(div().mt(px(6.0)).child(popover::dialog_body(theme, body)))
                .child(
                    div()
                        .mt(px(16.0))
                        ...
                        .gap(px(8.0))
                        .child(
                            popover::btn_ghost(theme, "Cancel", "organization-delete-cancel")
                                .id("organization-delete-cancel")
                                .on_click(cx.listener(|this, _, _, cx| {
                        ...
                        .child(
                            popover::btn_danger(theme, "Delete organization")
                                ...
                                .on_click(cx.listener(move |this, _, _, cx| {
                                    ...
                                    cx.emit(OrganizationPageEvent::RequestRuntimeChange(
                                        OrganizationMutation::Delete {
                                            organization_id: organization_id.clone(),
                                            label: organization_name.clone().into(),
                                        },
                                    ));
                                })),
                        ),
                ...
                .into_any_element();
            return Some(popover::modal("organization-delete-dialog", viewport, card));
        }
...
impl Render for OrganizationPage {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mut column = widgets::page_column()
            .child(widgets::page_header(&theme, "Organization", None))
            .child(widgets::page_subtitle(
                &theme,
                "Organizations are shared: every member sees and can drive every session in the organization.",
            ));

        if scope == Some(ProfileScope::Local) || !signed_in {
            return column.child(
                div()
                    .mt(px(16.0))
                    .text_size(px(13.0))
                    .text_color(theme.text_muted)
                    .child("Sign in to create or join an organization."),
            );
        }
        ...
        column = column
            .child(
                div()
                    ...
                    .items_center()
                    .justify_between()
                    .child(widgets::field_label(&theme, "Organizations"))
                    .child(
                        widgets::ghost_action(&theme)
            ...
            .child(self.render_organizations(&theme, cx))
            .child(
                div()
                    .mt(px(28.0))
