    ...
    match cache.map.get_mut(&key(namespace, device_id, path)) {
        ...
        None => {
            ...
            if let Some(image) = upload_alias_id8(path).and_then(|id8| {
                match cache.map.get(&alias_key(namespace, device_id, &id8)) {
                    Some(CacheEntry::Loaded { image, .. }) => Some(image.clone()),
                    _ => None,
                }
            }) {
                cache.insert_loaded(key(namespace, device_id, path), image.clone());
...
fn alias_key(namespace: &ProfileCacheNamespace, device_id: &str, id8: &str) -> CacheKey {
    key(namespace, device_id, &format!("upload-alias://{id8}"))
}
...
pub(crate) fn seed_attachment_alias(
    namespace: &ProfileCacheNamespace,
    device_id: &str,
    upload_id: &str,
    name: &str,
    ...
    let (_, device, path) = alias_key(namespace, device_id, &id8);
    store_loaded(namespace, &device, &path, name.to_string().into(), image);
}
