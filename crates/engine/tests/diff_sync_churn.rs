...
//!
//! Reconcile runs on every registry chat row change — including the writes
...
/// Block until the registry chat watch reflects `chat_id`'s presence/absence —
...
async fn chat_flap_keeps_entry_and_sustained_absence_removes_it() {
    ...
    core.registry.delete_chat("chat-1").expect("delete chat");
