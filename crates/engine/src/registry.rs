...
mod tests {
    ...
    /// A prefs file from the fixed-default allow-list era is ignored, not
    /// migrated (no-backward-compat fork): only the `disabled` key is read,
    /// so an unknown legacy key parses cleanly as the default prefs.
    #[test]
    fn legacy_allow_list_is_ignored_not_migrated() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::write(
        ...
        test_slot(&registry, HarnessId::Codex, true);
        registry.load_prefs(dir.path());
        // Enablement follows detection alone; the legacy key carries nothing.
        assert_eq!(
            registry.enabled_set(),
